...
export class MD5 {
     ...
     * @param resultCoding 摘要的编码方式（base64/hex），默认不传为hex。
     */
    static async hmac(data: string, symKey: cryptoFramework.SymKey = MD5.symKey, resultCoding: crypto.BhCoding = 'hex'): Promise<string> {
     ...
     * @param resultCoding 摘要的编码方式（base64/hex），默认不传为hex。
     */
    static hmacSync(data: string, symKey: cryptoFramework.SymKey = MD5.symKey, resultCoding: crypto.BhCoding = 'hex'): string {
     ...
     * @param len 自定义的数据拆分长度
     */
    static async hmacSegment(data: string, symKey: cryptoFramework.SymKey = MD5.symKey, resultCoding: crypto.BhCoding = 'hex', len: number = 128): Promise<string> {
     ...
     * @param len 自定义的数据拆分长度
     */
    static hmacSegmentSync(data: string, symKey: cryptoFramework.SymKey = MD5.symKey, resultCoding: crypto.BhCoding = 'hex', len: number = 128): string {
